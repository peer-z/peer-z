...
func IgdClear(peerPort uint16) {
	igd.Clear(peerPort)
}

func IgdIsForwardedUDP(peerPort uint16) (isFwd bool, err error) {
	isFwd, err = igd.IsForwardedUDP(peerPort)
	return
}

func UpnpDiscover() (err error) {
	err = nil
	if !*noUPnP {
...
func IgdExternalIP() (ip string, err error) {
	ip, err = igd.ExternalIP()
	return
}

func IgdForward(peerPort uint16, name string) error {
	return igd.Forward(peerPort, name)
}
...
func (peer Peer) MarshalPeerData(writer io.Writer, message peerMessage) error {
	...
	gob.Register(PeerInfos{})
	gob.Register(RerouteList{})
...
func messageHandler() {
	for {
		...
		case netMessage := <-messagesIn:
			...
			//fmt.Println("Message from", netMessage.addr, "content:",
			//    dump(netMessage.data, len(netMessage.data)))
			...
			message.Source.LocalIp = strings.Split(netMessage.addr.String(), ":")[0]
			...
			if message.Flags&MSG_ACK_REQUEST > 0 {
				...
				//reply.Destination.LocalIp = message.Source.LocalIp
				reply.Ttl = MAX_TTL
				reply.Counter = 0
				reply.send()
				Logln("Acked message...")
...
func (message peerMessage) String() string {
