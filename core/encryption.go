...
func (message *peerMessage) Encrypt(key Key, label []byte) (*peerMessage, error) {
    rng := rand.Reader
    encryptedContent, err := rsa.EncryptOAEP(crypto.SHA256.New(), rng, key.(*rsa.PublicKey), message.Content.Bytes(), label)
    if err != nil {
        return message, err
    }
    message.Content = bytes.NewBuffer(encryptedContent)
    return message, nil
}

func (message *peerMessage) Sign() (*peerMessage, error) {
    rng := rand.Reader
    hash := sha256.Sum256(message.Content.Bytes())
    signedContent, err := rsa.SignPKCS1v15(rng, privKey, crypto.SHA256, hash[:])
    message.Content = bytes.NewBuffer(signedContent)
    if err != nil {
        return message, err
    }
    return message, nil
